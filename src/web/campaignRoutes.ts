...
export function registerCampaignRoutes(
  ...
  server.post<{ Body: CampaignComposerInput }>(
    '/api/campaigns/simulate',
    async (request, reply) => {
      try {
  ...
  server.post<{ Body: CampaignComposerInput }>('/api/campaigns', async (request, reply) => {
    try {
      return reply
        .code(201)
        .send(campaigns.createDraft(request.body ?? ({} as CampaignComposerInput)));
    } catch (error) {
      return sendCampaignError(reply, error);
    }
  });

  server.put<{ Params: { id: string }; Body: CampaignComposerInput }>(
    '/api/campaigns/:id',
    async (request, reply) => {
      const id = Number(request.params.id);
      if (!Number.isSafeInteger(id) || id <= 0) {
      ...
      try {
        const campaign = await campaigns.updateDraft(
          id,
          request.body ?? ({} as CampaignComposerInput),
        );
  ...
  server.post<{ Params: { id: string }; Body: { confirmed?: boolean } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!campaign)
          return reply.code(409).send({ message: 'A campanha não está disponível como rascunho.' });
